...
class UnsignedSubtraction(IntegersScene):
	...
	def intro(self):
		...
		self.invY = MathTex(f"\ominus y = {self.numY}").next_to(self.y, DOWN)
		self.play(FadeIn(self.n), FadeIn(self.x), FadeIn(self.y))
		self.play(TransformFromCopy(self.y, self.invY))
	...
	def wheelAnim(self):
		...
		self.play(self.invY.animate.set_color(WHITE))
...
class SignedSubtraction(IntegersScene):
	...
	def math(self):
		...
		mathSumText = MathTex(f"s = {self.numX} + {self.numY}").next_to(self.invY, DOWN * 1).shift(RIGHT * 0.25)
