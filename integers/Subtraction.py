...
class UnsignedSubtraction(Scene):
	def construct(self):
		nbits = 2
		numX = 3
		numY = 2
		_numY = numY
		numY = 0 if numY == 0 else (2**nbits)-numY
		...
		self.play(FadeIn(uSubEqu))

		n = MathTex(f"n = {nbits}").to_corner(UL).shift(DOWN)
		x = MathTex(f"x = {numX}").next_to(n, DOWN)
		y = MathTex(f"y = {_numY}").next_to(x, DOWN)
		invY = MathTex(f"\ominus y = {numY}").next_to(y, DOWN)
		self.play(FadeIn(n), FadeIn(x), FadeIn(y))
		self.play(TransformFromCopy(y, invY))
		...
		self.play(invY.animate.set_color(GREEN))
		...
		self.play(invY.animate.set_color(WHITE))
...
class SignedSubtraction(Scene):
	def construct(self):
		nbits = 2
		numX = 1
		numY = -1
		_numY = numY
		numY = numY if numY == -(2**(nbits-1)) else -numY
		mathSum = numX + numY
		...
		mathSumText = MathTex(f"s = {numX} + {numY}").next_to(invY, DOWN * 1).shift(RIGHT * 0.25)
