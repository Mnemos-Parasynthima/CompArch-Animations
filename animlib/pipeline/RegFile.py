...
class RegFile(VGroup):
	def __init__(self, valw:int, dst:int, src1:int, src2:int, valA:int, valB:int, regfileHeight:float=2.5, regfileWidth:float=1.8):
		super().__init__()
		...
		self.wEnableArrow = Arrow(
			max_tip_length_to_length_ratio=0.15, color=RED
		).put_start_and_end_on(start=self.regfile.get_top()+UP, end=self.regfile.get_top())
		...
		).put_start_and_end_on(start=self.regfile.get_right()+(DOWN*0.15), end=self.regfile.get_right()+RIGHT+(DOWN*0.15))
		...
		self.valA = valA
		valAGroup = VGroup(self.valAArrow, self.valALabel, self.valAText)

		self.valBArrow = Arrow(
			max_tip_length_to_length_ratio=0.15
		).put_start_and_end_on(start=self.regfile.get_right()+(DOWN*0.8), end=self.regfile.get_right()+RIGHT+(DOWN*0.8))
		...
		self.valB = valB
		valBGroup = VGroup(self.valBArrow, self.valBLabel, self.valBText)

		labelTextArrowBuff = 0.02

		self.valWArrow = Arrow(
			max_tip_length_to_length_ratio=0.15
		).put_start_and_end_on(start=self.regfile.get_left()+LEFT+(UP), end=self.regfile.get_left()+(UP))
		...
		self.valw = valw
		valWGroup = VGroup(self.valWArrow, self.valWLabel, self.valWText)

		self.dstArrow = Arrow(
			max_tip_length_to_length_ratio=0.15
		).put_start_and_end_on(start=self.regfile.get_left()+LEFT+(UP*0.33), end=self.regfile.get_left()+(UP*0.33))
		...
		self.dst = dst
		dstGroup = VGroup(self.dstArrow, self.dstLabel, self.dstText)

		self.src1Arrow = Arrow(
			max_tip_length_to_length_ratio=0.15
		).put_start_and_end_on(start=self.regfile.get_left()+LEFT+(DOWN*0.33), end=self.regfile.get_left()+(DOWN*0.33))
		...
		self.src1 = src1
		src1Group = VGroup(self.src1Arrow, self.src1Label, self.src1Text)

		self.src2Arrow = Arrow(
			max_tip_length_to_length_ratio=0.15
		).put_start_and_end_on(start=self.regfile.get_left()+LEFT+(DOWN), end=self.regfile.get_left()+(DOWN))
		...
		self.src2 = src2
		src2Group = VGroup(self.src2Arrow, self.src2Label, self.src2Text)
